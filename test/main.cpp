...
int main()
{
    try
    {
        ...
        const std::vector<uint8_t> h = sha1::hash(test.begin(), test.end());
        ...
        const std::string b = base64::encode(test.begin(), test.end());
        ...
        const std::vector<uint8_t> b2 = base64::decode(b.begin(), b.end());

        if (b2 != test)
            throw std::runtime_error("Invalid decoded base64");

        const std::vector<uint8_t> d = md5::generate(test.begin(), test.end());
        ...
        const uint32_t f32 = fnv1::hash<uint32_t>(test.begin(), test.end());
        ...
        const uint64_t f64 = fnv1::hash<uint64_t>(test.begin(), test.end());
        ...
        const uint8_t c8 = crc8::generate(test.begin(), test.end());
        ...
        const uint32_t c32 = crc32::generate(test.begin(), test.end());
